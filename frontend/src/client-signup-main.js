import {
  getClientSignupInvite,
  getCurrentSession,
  onAuthStateChange,
  redeemClientSignupInvite,
  signInUser,
  signUpUser,
} from "./api.js";
...
function populateInviteDetails(invite) {
  ...
  const invitedCompany = document.getElementById("invitedCompany");
  const invitedCommunity = document.getElementById("invitedCommunity");
  const invitedExpires = document.getElementById("invitedExpires");
  if (invitedEmail) invitedEmail.textContent = email || "Unknown";
  if (invitedCompany) invitedCompany.textContent = invite?.company_name || "Unknown";
  if (invitedCommunity) invitedCommunity.textContent = invite?.community_name || "Unknown";
  ...
  if (signupFullName && fullName && !signupFullName.value.trim()) {
    signupFullName.value = fullName;
  }
}
...
async function validateInviteToken() {
  ...
  try {
    const invite = await getClientSignupInvite(state.inviteToken);
...
async function tryRedeemInvite(fullNameFallback = null) {
  ...
  try {
    ...
    await redeemClientSignupInvite({
      inviteToken: state.inviteToken,
      fullName,
    });
    setMessage("Client access granted. Redirecting to client home...", "success");
    setTimeout(() => {
